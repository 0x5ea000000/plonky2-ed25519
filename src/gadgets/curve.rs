...
use plonky2_crypto::biguint::{BigUintTarget, GeneratedValuesBigUint};
...
pub trait CircuitBuilderCurve<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn precompute_wnaf_window<C: Curve>(
        &mut self,
        p: &AffinePointTarget<C>,
    ...
    fn fixed_base_curve_scalar_mul_comb<C: Curve>(
        &mut self,
        n: &Vec<BoolTarget>,
    ) -> AffinePointTarget<C>;
}
    ...
    for CircuitBuilder<F, D>
{
    ...
    fn point_decompress<C: Curve>(
        &mut self,
        compressed_point: &Vec<BoolTarget>,
    ) -> AffinePointTarget<C> {
    ...
    fn precompute_wnaf_window<C: Curve>(
        ...
        let big_one =
            <C as Curve>::ScalarField::from_noncanonical_biguint(BigUint::from_str("1").unwrap());
    ...
    fn precompute_comb_array<C: Curve>(&mut self) -> Vec<AffinePointTarget<C>> {
        ...
        for i in 1..16 {
            ...
            precomp_array.push(
                mul_naive(
                    C::ScalarField::from_noncanonical_biguint(val),
    ...
    for CurvePointDecompressionGenerator<F, D, C>
{
    ...
    fn run_once(
        ...
        let bits = self
            .compressed
            .iter()
            .map(|bt| witness.get_bool_target(bt.clone()))
            .collect::<Vec<_>>();
        ...
        out_buffer.set_biguint_target(
            &self.new_point.x.value,
            &decompressed_point.x.to_canonical_biguint(),
        );
        out_buffer.set_biguint_target(
            &self.new_point.y.value,
            &decompressed_point.y.to_canonical_biguint(),
        );
